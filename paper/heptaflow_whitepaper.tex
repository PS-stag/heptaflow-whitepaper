...
% --- OLD FONT SETUP (pdflatex) ---
% \usepackage[T1]{fontenc}
% \usepackage[utf8]{inputenc}
% \usepackage{lmodern}


% --- NEW FONT SETUP (xelatex) ---
\usepackage{fontspec}
...
Many organizations are currently undergoing a transition from traditional on-premise data warehouse environments \textendash{} often built on Oracle, Informatica, DataStage, or similar technologies \textendash{} towards modern, cloud-based data platforms such as Databricks.
...
\begin{itemize}
  \item ETL tools (e.g.\ Informatica/DataStage workflows and mappings),
...
\begin{itemize}
  ...
  \item temporal validity handling (e.g.\ slowly changing dimensions, mono- and bi-temporal patterns, technical vs. business time),
  \item implicit filtering logic or data quality rules.
\end{itemize}

These semantics are rarely documented in a structured way. Instead, they are often hidden within code, tool configurations, or developer knowledge.
...
A different approach is required \textendash{} one that treats transformation logic as a first-class, structured artifact rather than an implementation detail.
...
For example:
\begin{itemize}
  \item DataStage/Informatica jobs or Oracle-based ETL processes,
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=1.9cm]
    ...
    % --- Modern Platform (lighter) ---
    \node (modern) [hfbox, fill=blue!1] 
    {\textbf{Modern Data Platform}\\(Databricks / Spark / Delta Lake)};

    % --- Label ---
    \node (txt1) [below=0.5cm of modern, hfsmall] {Generated Pipelines};

    % --- Generator (highlighted core) ---
    \node (generator) [hfbox, fill=hfblue!10, below=1.0cm of txt1] 
    {\textbf{Transformation / Generator Layer}\\(XSLT / Rules / Templates / Mapping Engine)};

    % --- Label ---
    \node (txt2) [below=0.5cm of generator, hfsmall] {Structured Metadata};

    % --- Metadata Layer ---
    \node (meta) [hfbox, below=1.0cm of txt2] 
    {\textbf{Metadata \& Representation Layer}\\(XML / ETL Exports / Logical Transformation Model)};

    % --- Legacy Layer (slightly darker) ---
    \node (legacytxt) [hfbox, fill=black!3, below=1.0cm of meta, align=center] 
    {\textbf{Legacy ETL Systems}\\(Oracle / DataStage / Informatica / SQL / Procedures)};
  ...
  \caption{Generator-based lift-and-shift architecture for transforming legacy ETL into modern data platforms.}
...
\begin{itemize}
  \item Informatica/DataStage job definitions (XML exports),
  \item Oracle-based ETL logic (PL/SQL, SQL scripts),
  \item Informatica/DataStage workflows and mappings.
\end{itemize}
...
By contrast, modern data architectures \textendash{} often described in the context of Data Mesh \textendash{}emphasize \emph{data products}:
\begin{itemize}
  \item domain-owned,
